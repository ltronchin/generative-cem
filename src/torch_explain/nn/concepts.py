...
class ConceptEmbedding(torch.nn.Module):
    def __init__(
        ...
        # We create a context generator for each concept.
        ...
        # We create a SHARED concept probability predictor.
    ...
    def forward(self, x):
        ...
        # We iterate over the concepts.
        for i, context_gen in enumerate(self.concept_context_generators):
            context = context_gen(x)
            c_pred = self.concept_prob_predictor(context) # Predict the probability of the concept being true.
...
class ConceptEmbeddingIntervention(torch.nn.Module):
    def __init__(
            self,
            in_features,
            n_concepts,
        ...
        self.concept_prob_predictor = torch.nn.Sequential(
            torch.nn.Linear(2 * emb_size, 1),
            torch.nn.Sigmoid(), # todo here adapt to regression
        )
